...
export default function TexCoordDrawer(mesh) {
    ...
    surface.addEventListener('keydown', (event) => {
        ...
        switch (event.key) {
            case 'w': 
                point[1] = Math.min(point[1] + step, 1.0);
                break;
            case 's': 
                point[1] = Math.max(point[1] - step, 0.0);
                break;
            case 'a': 
                point[0] = Math.max(point[0] - step, 0.0);
                break;
            case 'd': 
                point[0] = Math.min(point[0] + step, 1.0);
                break;
            default:
                return; 
        }
        ...
        document.getElementById('coordinates').textContent = `Coordinates: (${point[0].toFixed(2)}, ${point[1].toFixed(2)})`;
